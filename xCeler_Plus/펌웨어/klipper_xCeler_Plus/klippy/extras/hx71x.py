...
class HX71xBase:
    def __init__(self, config, sensor_type,
        ...
        # Load cell threshold 읽기
        raw_threshold = config.getint("loadcell_threshold", 0)
        if raw_threshold <= 0:
            self.loadcell_threshold = 8000
        else:
            self.loadcell_threshold = raw_threshold
        gcode = config.get_printer().lookup_object("gcode")
        gcode.respond_info(
            f"[HX711] loadcell threshold ({self.loadcell_threshold})"
        )
        ## Bulk Sensor Setup
        ...
        mcu.add_config_cmd(
            "config_hx71x oid=%d gain_channel=%d dout_pin=%s "
            "sclk_pin=%s threshold=%d"
            % (self.oid, self.gain_channel, self.dout_pin,
                self.sclk_pin, self.loadcell_threshold)
        )
    ...
    def _start_measurements(self):
        ...
        rest_ticks = 0
        #rest_ticks = self.mcu.seconds_to_clock(0.4)
