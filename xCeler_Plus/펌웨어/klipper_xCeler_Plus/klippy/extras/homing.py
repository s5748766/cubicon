...
class Homing:
    ...
    def home_rails(self, rails, forcepos, movepos):
        try:
            ...
            rail_names = [rail.get_name().lower()[-1] for rail in rails]
            #self.printer.lookup_object("gcode").respond_info(
            #    f"[HOMING] rails = {rail_names}"
            #)
            self.printer.lookup_object("gcode").respond_info(
                f"[HOMING] rails = {rail_names}"
            )
            axes = [rail.get_name().lower()[-1] for rail in rails]
            if 'z' in axes:
                self.printer.send_event("home_z_start", self)
            ...
            self.printer.send_event("homing_end", self)
class PrinterHoming:
