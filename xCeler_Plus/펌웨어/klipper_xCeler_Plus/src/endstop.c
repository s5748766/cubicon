...
struct endstop {
    ...
    struct trsync *ts;
    uint8_t flags, sample_count, trigger_count, trigger_reason, is_z;
};

enum { ESF_PIN_HIGH=1<<0, ESF_HOMING=1<<1 };

static uint_fast8_t endstop_oversample_event(struct timer *t);
static uint8_t endstop_index = 0;
extern uint8_t hx711_flag;
...
endstop_event(struct timer *t)
{
    ...
    uint8_t val = 0;
    //if ((uint32_t)e->pin.regs == 0x50000800 && e->pin.bit == (1 << 14))
    if (e->is_z)
    {
        val = hx711_flag;
    }
...
command_config_endstop(uint32_t *args)
{
    ...
    if (endstop_index == endstop_z) e->is_z = 1;
    else    e->is_z = 0;

    endstop_index++;

    //debug_printf(
    // "Endstop[%d] -> regs=0x%x, bit=%d\r\n",
    // args[0],(uint32_t)e->pin.regs,e->pin.bit
    // );
}
