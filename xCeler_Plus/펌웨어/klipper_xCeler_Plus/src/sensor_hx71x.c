...
#include <stdint.h>
#include <stdlib.h>
#include "stm32/uart_debug.h" //debug_printf
...
static uint8_t hx711_state = 0;   // 0: BUSY, 1: COMPLETE, 2: READY
static int32_t temp_data = 0;
static int32_t loadcellRefValue = 0;
static int32_t loadcellCurValue = 0;
uint8_t hx711_flag = 0;
    ...
    switch (hx711_state)
    {
        ...
        case 2: // READY
            ...
            if (bit_counter >= (25 * 2))
            {
                ...
                {
                    temp_data = (~temp_data + 1) & 0x007FFFFF;
                    data = -(int32_t)temp_data;
                }
                else
                {
                    data = (int32_t)temp_data;
                }
                loadcellCurValue = data;
                temp_data = 0;
                bit_counter = 0;
                hx711_state = 0;
                if(loadcellRefValue != 0)
                {
                    if (abs(loadcellCurValue - loadcellRefValue)
                         > hx71x->loadcellthreshold)
                    {
                        hx711_flag = 1;
                    }
                    else
                    ...
                    loadcellRefValue = loadcellCurValue;
                }
                else    loadcellRefValue =  loadcellCurValue;
...
command_query_hx71x(uint32_t *args)
{
    ...
    if (!hx71x->rest_ticks) {
        // End measurements
        debug_printf("HX711_EVENT_OFF\r\n");
        gpio_out_write(hx71x->sclk, 1); // put chip in power down state
        return;
    }
    // Start new measurements
    debug_printf("HX711_EVENT_ON\r\n");
    ...
    //debug_printf("hx71x->rest_ticks = %d\r\n",hx71x->rest_ticks);
...
hx71x_capture_task(void)
{
    if(cprf)
        debug_printf(
            "loadcellCurValue = %d / hx711_flag = %d\r\n",
            loadcellCurValue, hx711_flag
        );
